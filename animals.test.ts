...
import { check, group, sleep, JSONObject, fail } from "k6";
import {
  randomItem,
  randomIntBetween,
  //@ts-ignore
...
// Use the Animal interface for all animal objects
function getRandomNewAnimal(): Omit<Animal, "id"> {
  // Pick a random animal from the JSON list and remove the id
  const baseAnimal = randomItem(animals);
  const { id, ...rest } = baseAnimal;
  return { ...rest };
}
...
export function setup(): SetupData {
  // Validate the main endpoint before running the test
...
export default function (data: SetupData) {
  ...
  group("Get all animals", function () {
    ...
    resAll.status === 200 ? getAllSuccess.add(1) : getAllErrors.add(1);
  ...
  group("Get animal by ID", function () {
    ...
    resOne.status === 200 ? getByIdSuccess.add(1) : getByIdErrors.add(1);
  ...
  group("Create a new animal", function () {
    const newAnimal: Omit<Animal, "id"> = getRandomNewAnimal();
    ...
    res.status === 201 ? createSuccess.add(1) : createErrors.add(1);
    if (res.status === 201) {
      try {
  ...
  group("Update an existing animal", function () {
    ...
    const updated: Omit<Animal, "id"> = getRandomNewAnimal();
    const res = http.put(
    ...
    res.status === 200 ? updateSuccess.add(1) : updateErrors.add(1);
...
export function handleSummary(data: JSONObject): JSONObject {
  return {
    "report/load-testing.html": htmlReport(data, {
